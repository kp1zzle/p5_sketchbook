/* [this.p5.videorecorder] Version - 0.2.1 - 03-07-2022
https://github.com/calebfoss/this.p5.videorecorder */
...
export class VideoRecorder {
    ...
    constructor(s, input, format = "webm") {
        this.p5 = s;
        this.format = format;
        this.input = input;
    }
    ...
    set input(input) {
        ...
        if (input === undefined) {
            if (typeof this.p5.drawingContext?.canvas === "undefined")
                console.error("VideoRecorder couldn't find canvas to record");
            else this.input = this.p5.drawingContext.canvas;
    ...
    set format(format) {
        ...
        if (VideoRecorder.isSupported(format) == false)
            return console.error(
    ...
    canRecord(input) {
        return VideoRecorder.canRecord(input);
    }
    ...
    #inputToStream(input) {
        ...
        if (input instanceof this.p5.Element)
            return this.#mediaElementToStream(input.elt);
    }
    isSupported(format) {
        return VideoRecorder.isSupported(format);
    }
    ...
    save(filename) {
        ...
        [filename, extension] = this.p5._checkFileExtension(
            filename,
            extension
        );
        this.p5.downloadFile(this.#blob, filename, extension);
    }
    start() {
        this.erase();
        this.#recorder.start();
